...
const useAuth = () => {
  ...
  api.interceptors.response.use(
    ...
    async (error) => {
      ...
      if (error?.response?.status === 401) {
        /** Não remover token quando `skipAuth401Handler` está no logout — o handleLogout faz o cleanup. */
        if (!originalRequest?.skipAuth401Handler) {
  ...
  /**
   * @param {*} userData email/senha
   * @param {{ redirectTo?: string, requireSuper?: boolean }} opts — ex.: login do backoffice: `{ redirectTo: '/admin', requireSuper: true }`
   */
  const handleLogin = async (userData, opts = {}) => {
    const { redirectTo, requireSuper } = opts;
    setLoading(true);

    try {
      const { data } = await api.post("/auth/login", userData);

      if (requireSuper === true && !data.user.super) {
        toast.error(i18n.t("adminLogin.deniedSuper"));
        setLoading(false);
        return;
      }
      ...
      if (before === true) {
        ...
        history.push(redirectTo || "/tickets");
        setLoading(false);
      } else {
        api.defaults.headers.Authorization = `Bearer ${data.token}`;
        setIsAuth(true);
...
Entre em contato com o Suporte para mais informações! `);
  ...
  const handleLogout = async () => {
    ...
    // Limpar socket antes de tentar invalidar sessão na API (evita callbacks paralelos)
    if (socket) {
      listenersRef.current.forEach((eventName) => {
        if (socket.off) {
          socket.off(eventName);
    ...
    try {
      await api.delete("/auth/logout", { skipAuth401Handler: true });
    } catch (_) {
      // Token expirado, cookie ausente ou sessão já invalidada — saída local igual
    } finally {
      setIsAuth(false);
      setUser({});
      setSocket(null);
