...
export default function Options(props) {
  ...
  // Fechar ticket ao transferir para outro departamento
...
const [loadingCopyContactPrefix, setLoadingCopyContactPrefix] = useState(false);
  ...
  const [loadingWavoipUrl, setLoadingWavoipUrl] = useState(false);

  const [wavoipUsername, setWavoipUsername] = useState("");
  const [loadingWavoipUsername, setLoadingWavoipUsername] = useState(false);

  const [wavoipPassword, setWavoipPassword] = useState("");
  const [loadingWavoipPassword, setLoadingWavoipPassword] = useState(false);
  ...
  useEffect(() => {
    for (const [key, value] of Object.entries(settings)) {
      ...
      if (key === "wavoipUrl") setWavoipUrl(value || "");
      if (key === "wavoipUsername") setWavoipUsername(value || "");
      if (key === "wavoipPassword") setWavoipPassword(value || "");
    }
  }, [settings]);
  ...
  async function handleChangeWavoipUrl(value) {
    setWavoipUrl(value);
    setLoadingWavoipUrl(true);
    await update({
      column: "wavoipUrl",
      data: value,
    });
    setLoadingWavoipUrl(false);
    toast.success("URL Wavoip atualizada com sucesso!");
  }

  async function handleChangeWavoipUsername(value) {
    setWavoipUsername(value);
    setLoadingWavoipUsername(true);
    await update({
      column: "wavoipUsername",
      data: value,
    });
    setLoadingWavoipUsername(false);
    toast.success("Usuário Wavoip atualizado com sucesso!");
  }

  async function handleChangeWavoipPassword(value) {
    setWavoipPassword(value);
    setLoadingWavoipPassword(true);
    await update({
      column: "wavoipPassword",
      data: value,
    });
    setLoadingWavoipPassword(false);
    toast.success("Senha Wavoip atualizada com sucesso!");
  }
    ...
    shrink: true,
      ...
      <Grid spacing={3} container>
        {isSuper() ?
          ...
          >
            <Tab label='Recebimentos de Faturas ' />
          </Tabs>
          : null}
      </Grid>
        ...
        <Grid xs={12} sm={12} md={12} item>
          <FormControl className={classes.selectContainer}>
            <TextField
              ...
              name='wavoipUrl'
              margin='dense'
              label='URL Wavoip (ex: https://api.wavoip.com)'
              variant='outlined'
              value={wavoipUrl}
              onChange={async (e) => {
                handleChangeWavoipUrl(e.target.value);
              }}
              placeholder='https://api.wavoip.com'
            ></TextField>
            <FormHelperText>
              {loadingWavoipUrl && 'Atualizando...'}
            </FormHelperText>
          </FormControl>
        </Grid>
        <Grid xs={12} sm={6} md={6} item>
          <FormControl className={classes.selectContainer}>
            <TextField
              ...
              name='wavoipUsername'
              margin='dense'
              label='Email/Usuário Wavoip'
              variant='outlined'
              value={wavoipUsername}
              onChange={async (e) => {
                handleChangeWavoipUsername(e.target.value);
              }}
              type='email'
            ></TextField>
            <FormHelperText>
              {loadingWavoipUsername && 'Atualizando...'}
            </FormHelperText>
          </FormControl>
        </Grid>
        <Grid xs={12} sm={6} md={6} item>
          <FormControl className={classes.selectContainer}>
            <TextField
              ...
              variant='outlined'
              type='password'
              value={wavoipPassword}
              onChange={async (e) => {
                handleChangeWavoipPassword(e.target.value);
              }}
            ></TextField>
            <FormHelperText>
              {loadingWavoipPassword && 'Atualizando...'}
            </FormHelperText>
          </FormControl>
        </Grid>
      </Grid>
