...
// Lista de modelos Gemini suportados (baseado na documentação oficial mais recente)
const geminiModels = [
  "gemini-3-flash",
  "gemini-3-pro",
  "gemini-2.5-flash",
...
const FlowBuilderGeminiModal = ({ open, onSave, data, onUpdate, close }) => {
  ...
  const getModelDisplayName = (model) => {
    const modelNames = {
      "gemini-3-pro": "🧠 Gemini 3 Pro (Mais Inteligente)",
      "gemini-3-flash": "⚡ Gemini 3 Flash (Mais Equilibrado)",
      ...
      "gemini-2.0-flash": "⚡ Gemini 2.0 Flash (Segunda Geração)",
      "gemini-2.0-flash-lite": "💨 Gemini 2.0 Flash Lite (Rápido)"
    };
                  ...
                  <AccordionDetails className={classes.accordionDetails}>
                    ...
                    {values.flowMode === "temporary" && (
                      <div className={classes.temporarySettings}>
                        ...
                        <Field
                          as={TextField}
                          label="ID do departamento (para transferência)"
                          name="queueId"
                          error={touched.queueId && Boolean(errors.queueId)}
                          helperText="ID do departamento para onde transferir se usuário pedir atendente (0 = não transferir)"
                          variant="outlined"
                          margin="dense"
                          fullWidth
                          type="number"
