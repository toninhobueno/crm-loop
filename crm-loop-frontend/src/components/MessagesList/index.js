...
const MessagesList = ({
  ...
  useEffect(() => {
    ...
    const delayDebounceFn = setTimeout(() => {
      const fetchMessages = async () => {
        ...
        try {
          const shouldLoadRecentConversations = ticketStatus === "pending" && pageNumber === 1;
          const { data } = await api.get("/messages/" + ticketId, {
            params: {
              pageNumber,
              selectedQueues: JSON.stringify(selectedQueuesMessage),
              loadRecentConversations: shouldLoadRecentConversations ? "true" : "false",
              conversationLimit: shouldLoadRecentConversations ? 10 : undefined,
              messagesPerConversation: shouldLoadRecentConversations ? 50 : undefined
            },
          });
  ...
  }, [pageNumber, ticketId, selectedQueuesMessage, ticketStatus]);
  ...
  const checkMessageMedia = (message) => {
    ...
    // Vídeos
      ...
      // Se não tem URL ainda, mostrar placeholder
      if (!message.mediaUrl) {
        return (
          <div style={{ 
            maxWidth: "400px", 
          ...
          return (
            <React.Fragment key={message.id}>
              ...
              >
                ...
                {(message.mediaUrl || message.mediaType === "locationMessage" || message.mediaType === "contactMessage" || message.mediaType === "template" || message.mediaType === "adMetaPreview" || message.mediaType === "image" || message.mediaType === "audio" || message.mediaType === "video" || message.mediaType === "document"
          ...
          return (
            <React.Fragment key={message.id}>
              ...
              >
                ...
                {(message.mediaUrl || message.mediaType === "locationMessage" || message.mediaType === "contactMessage" || message.mediaType === "template" || message.mediaType === "image" || message.mediaType === "audio" || message.mediaType === "video" || message.mediaType === "document"
                ) && checkMessageMedia(message)}
                <div
