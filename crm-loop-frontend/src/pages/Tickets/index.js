...
import { Select, MenuItem, FormControl, InputLabel, Chip } from "@material-ui/core";
...
const Ticket = () => {
  ...
  useEffect(() => {
    const loadKanbanStages = async () => {
      try {
        ...
        setKanbanStages(data.lista || []);
      } catch (err) {
        console.error('Erro ao carregar etapas do Kanban:', err);
      }
    };
    loadKanbanStages();
  }, []);
  ...
  const handleKanbanStageChange = async (e) => {
    const stageId = e.target.value;
    try {
      // Remover etapa atual se houver
      if (ticket.tags && ticket.tags.length > 0) {
        await api.delete(`/ticket-tags/${ticket.id}`);
      }
      
      // Adicionar nova etapa se não for vazio
      if (stageId && stageId !== '') {
        await api.put(`/ticket-tags/${ticket.id}/${stageId}`);
      }
  ...
  return (
    <div className={classes.root} id="drawer-container">
      ...
      >
        ...
        {/* </div> */}
        <Paper style={{ padding: 8, display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          <TagsContainer contact={contact} />
          {ticket && ticket.id && (
            <FormControl size="small" variant="outlined" style={{ minWidth: 150 }}>
              <InputLabel>Etapa Kanban</InputLabel>
              <Select
                value={ticket.tags && ticket.tags.length > 0 ? ticket.tags[0].id : ''}
                onChange={handleKanbanStageChange}
                label="Etapa Kanban"
                renderValue={(value) => {
                  if (!value) return 'Sem Etapa';
                  const stage = kanbanStages.find(s => s.id === value);
                  ...
                  return (
                    <Chip
                      ...
                      label={stage.name}
                      style={{
                        backgroundColor: stage.color || '#757575',
                        color: '#FFF',
                        height: 20,
                        fontSize: '0.75rem',
                      }}
