...
const CallHistoricals = () => {
  ...
  useEffect(() => {
    dispatch({ type: "RESET" });
    setPageNumber(1);
  }, [searchParam, statusFilter, directionFilter, dateFilter]);
  ...
  }, [searchParam, pageNumber, statusFilter, directionFilter, dateFilter]);
  ...
  // Buscar token Wavoip diretamente da tabela Whatsapps (name, number, wavoip)
  useEffect(() => {
    const fetchWavoipToken = async () => {
      try {
        ...
        console.error("Erro ao buscar token Wavoip:", err);
        setWavoipToken(null);
      }
    };
    fetchWavoipToken();
  ...
  const handleMakeCall = async (call) => {
    ...
    if (call.url) {
      window.open(call.url, "wavoip", "toolbar=no,scrollbars=no,resizable=no,top=500,left=500,width=500,height=700");
      return;
    }

    // Caso contrário, buscar token e montar URL
    let token = call.token_wavoip || wavoipToken;
    
    if (!token) {
      try {
        const { data } = await api.get("/call/whatsapp/wavoip");
        token = data?.whatsapp?.wavoip || null;
        if (token) {
          setWavoipToken(token);
        }
      } catch (err) {
              ...
              <Grid xs={12} sm={6} md={2} item>
                <FormControl fullWidth variant="outlined" size="small">
                  ...
                  >
                    ...
                    <MenuItem value="NO_ANSWER">Não Atendida</MenuItem>
              ...
              <Grid xs={12} sm={6} md={2} item>
                <FormControl fullWidth variant="outlined" size="small">
                  ...
                  >
                    ...
                    <MenuItem value="OUTCOMING">Saída</MenuItem>
                  </Select>
                </FormControl>
              </Grid>
          ...
          <TableBody>
            <>
              {callHistory.filter(call => {
                // Aplicar filtros locais para a exibição
                if (statusFilter !== "all" && call.devices?.status !== statusFilter) return false;
                if (directionFilter !== "all" && call.devices?.direction !== directionFilter) return false;
                if (dateFilter) {
                  const callDate = new Date(call.createdAt || call.devices?.created_date);
                  const filterDate = new Date(dateFilter);
                  if (callDate.toDateString() !== filterDate.toDateString()) return false;
                }
                return true;
              }).map((call) => (
                <TableRow key={call.id || call.devices?.id} hover>
                  <TableCell align="center">
                  ...
                  <TableCell align="center">
                    {call.phone_to && (call.url || wavoipToken || call.token_wavoip) && (
                      <IconButton
                        size="small"
                        className={classes.callButton}
              ...
              {!loading && callHistory.filter(call => {
                if (statusFilter !== "all" && call.devices?.status !== statusFilter) return false;
                if (directionFilter !== "all" && call.devices?.direction !== directionFilter) return false;
                if (dateFilter) {
                  const callDate = new Date(call.createdAt || call.devices?.created_date);
