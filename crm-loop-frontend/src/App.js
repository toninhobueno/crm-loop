...
const App = () => {
  ...
  const [primaryColorDark, setPrimaryColorDark] =
    useState(appColorLocalStorage);
  const [listItemBgColor, setListItemBgColor] = useState(
    localStorage.getItem("listItemBgColor") || "#ffffff"
  );
  ...
  const colorMode = useMemo(
    ...
    [appLogoLight, appLogoDark, appLogoFavicon, appName, listItemBgColor, mode]
  );
      ...
      createTheme(
        {
          ...
          overrides: {
            // Botões tipo produto SaaS: cantos marcados + reflexo superficial
            MuiButton: {
              root: {
                borderRadius: 12,
                textTransform: 'none',
              ...
              contained: {
                ...
                boxShadow:
                  mode === 'light'
                    ? '0 2px 8px rgba(15, 23, 42, 0.1), inset 0 1px 0 rgba(255,255,255,0.35)'
                    : '0 2px 12px rgba(0, 0, 0, 0.35), inset 0 1px 0 rgba(255,255,255,0.12)',
                '&::before': {
                  content: '""',
                  position: 'absolute',
                  top: 0,
                  ...
                  background:
                    mode === 'light'
                      ? 'linear-gradient(180deg, rgba(255,255,255,0.38) 0%, rgba(255,255,255,0.06) 45%, transparent 100%)'
  ...
  useEffect(() => {
    ...
    getPublicSetting("listItemBgColor")
      .then((color) => {
        setListItemBgColor(color || "#ffffff");
      })
      .catch((error) => {
        console.log("Error reading setting", error);
      });
