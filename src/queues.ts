...
async function handleDispatchCampaign(job) {
  try {
    ...
    // Verificar provider - UAZApi ou Baileys
    const isUAZApi = campaign.whatsapp.provider === "uazapi";
    let wbot: any = null;
    let uazapi: any = null;

    if (isUAZApi) {
      // Usar UAZApi
      const { getUAZApi } = await import("./libs/uazapi");
      try {
        uazapi = getUAZApi(campaign.whatsapp.id);
        logger.info(`[CAMPAIGN] Usando UAZApi para campanha ${campaignId}`);
      } catch (err) {
        logger.error(`[CAMPAIGN] UAZApi não inicializada para campanha ${campaignId}`);
        return;
      }
    } else {
      // Usar Baileys
    ...
    if (campaign.openTicket === "enabled") {
      ...
      if (whatsapp.status === "CONNECTED") {
        if (campaign.confirmation && campaignShipping.confirmation === null) {
          ...
          if (isUAZApi && uazapi) {
            await uazapi.sendTextMessage(campaignShipping.number, `\u200c ${campaignShipping.confirmationMessage}`);
          } else if (wbot) {
          ...
          if (!campaign.mediaPath) {
            console.log(`[DISPATCH-CAMPAIGN] Enviando mensagem de texto para: ${chatId} (UAZApi: ${isUAZApi})`);
            console.log(`[DISPATCH-CAMPAIGN] Conteúdo da mensagem: ${campaignShipping.message}`);

            if (isUAZApi && uazapi) {
              // Enviar via UAZApi
              await uazapi.sendTextMessage(campaignShipping.number, `\u200c ${campaignShipping.message}`);
              console.log(`[DISPATCH-CAMPAIGN] Mensagem enviada via UAZApi`);
            } else if (wbot) {
              // Enviar via Baileys
          ...
          if (campaign.mediaPath) {
            ...
            if (isUAZApi && uazapi) {
              ...
              const mimeType = campaign.mediaName?.split('.').pop() || 'application/octet-stream';
              let mediaType = 'document';
              if (mimeType.match(/jpg|jpeg|png|gif|webp/i)) mediaType = 'image';
              else if (mimeType.match(/mp4|avi|mov|webm/i)) mediaType = 'video';
              else if (mimeType.match(/mp3|ogg|wav|m4a/i)) mediaType = 'audio';
              
              // Ler arquivo e converter para base64
              const fileBuffer = fs.readFileSync(filePath);
              const base64File = `data:${mimeType};base64,${fileBuffer.toString("base64")}`;
              
              await uazapi.sendMediaMessage(
              ...
              console.log(`[DISPATCH-CAMPAIGN] Mídia enviada via UAZApi`);
            } else if (wbot) {
              // Enviar mídia via Baileys
      ...
      if (campaign.confirmation && campaignShipping.confirmation === null) {
        ...
        if (!campaign.mediaPath) {
          console.log(`[DISPATCH-CAMPAIGN] Enviando mensagem SEM ticket para: ${chatId} (UAZApi: ${isUAZApi})`);
          console.log(`[DISPATCH-CAMPAIGN] Conteúdo da mensagem: ${campaignShipping.message}`);

          if (isUAZApi && uazapi) {
            await uazapi.sendTextMessage(campaignShipping.number, campaignShipping.message);
            console.log(`[DISPATCH-CAMPAIGN] Mensagem enviada via UAZApi (sem ticket)`);
          } else if (wbot) {
            const sentMessage = await wbot.sendMessage(getJidOf(chatId), {
              text: campaignShipping.message
            });
        ...
        if (campaign.mediaPath) {
          ...
          if (isUAZApi && uazapi) {
            ...
            console.log(`[DISPATCH-CAMPAIGN] Mídia enviada via UAZApi (sem ticket)`);
          } else if (wbot) {
            const options = await getMessageOptions(
              campaign.mediaName,
              filePath,
