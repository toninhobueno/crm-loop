 ...
 * Execute: node dist/scripts/add-floup-columns.js
 * Ou após build: npm run build && node dist/scripts/add-floup-columns.js
 */
...
async function addFloupColumns() {
  ...
  try {
    if (dialect === 'postgres') {
      ...
      const existingColumns = colResults.map((row) => row.column_name.toLowerCase());
      conditionExists = existingColumns.includes('condition');
      conditionValueExists = existingColumns.includes('conditionvalue');
    } else {
      const tableDescription = await queryInterface.describeTable(actualTableName);
