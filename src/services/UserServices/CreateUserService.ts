...
interface Request {
  ...
  companyId?: number;
  profile?: string;
  /** Só definido pelo backend quando o criador já é `super` (master). */
  grantSuperMaster?: boolean;
  startWork?: string;
  endWork?: string;
  whatsappId?: number;
  allTicket?: string;
...
const CreateUserService = async ({
  ...
  grantSuperMaster = false
}: Request): Promise<Response> => {
  if (companyId !== undefined && !grantSuperMaster) {
  ...
  const user = await User.create(
    {
      ...
      companyId,
      profile,
      super: !!grantSuperMaster,
      startWork,
      endWork,
      whatsappId: whatsappId || null,
      allTicket,
      defaultTheme,
      defaultMenu: defaultMenu ?? "open",
      allowGroup,
      allHistoric,
      allUserChat,
      userClosePendingTicket,
