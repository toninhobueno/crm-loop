...
const buildWhereCondition = async ({
  ...
  // FILTRO INTELIGENTE DE CARTEIRA
  // Usuários do tipo "user" veem:
  // 1. Contatos SEM carteira (disponíveis para todos)
  // 2. Contatos DA SUA carteira (já atribuídos a ele)
  if (DirectTicketsToWallets && userProfile && userProfile.profile === "user" && userId) {
    // Adiciona filtro de carteira usando Op.and para não sobrescrever condições existentes
    whereCondition = {
      [Op.and]: [
        whereCondition,
        {
          [Op.or]: [
            // Contatos SEM carteira (contactId não existe em ContactWallets)
            {
              id: {
                [Op.notIn]: Sequelize.literal(`(SELECT DISTINCT "contactId" FROM "ContactWallets" WHERE "companyId" = ${companyId})`)
              }
            },
            // OU contatos DA CARTEIRA deste usuário
            {
              id: {
