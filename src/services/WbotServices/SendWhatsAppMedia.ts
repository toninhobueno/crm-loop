...
const SendWhatsAppMedia = async ({
  ...
  try {
    ...
    const SendUAZApiMedia = (await import("../UAZApi/SendUAZApiMedia")).default;
    const SendNotificameHubMedia = (await import("../NotificameHub/SendNotificameHubMedia")).default;
    
    const whatsapp = await Whatsapp.findByPk(ticket.whatsappId);
    if (whatsapp?.provider === "uazapi") {
      logger.info(`[SendWhatsAppMedia] Redirecting to UAZApi for whatsappId ${ticket.whatsappId}`);
      return await SendUAZApiMedia({ media, ticket, body, isPrivate, isForwarded }) as any;
    }
    if (whatsapp?.provider === "notificamehub") {
      logger.info(`[SendWhatsAppMedia] Redirecting to NotificameHub for whatsappId ${ticket.whatsappId}`);
      return await SendNotificameHubMedia({ media, ticket, body, isPrivate, isForwarded }) as any;
    }
