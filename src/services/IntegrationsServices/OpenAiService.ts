...
const handleGeminiRequest = async (
  ...
  try {
    console.log(`[GEMINI] Iniciando requisição - Modelo: ${aiSettings.model}`);
    console.log(`[GEMINI] API Key presente: ${!!aiSettings.apiKey}`);
    console.log(`[GEMINI] Mensagem: ${newMessage.substring(0, 100)}...`);
    ...
    if (!aiSettings.model) {
      throw new Error("Modelo do Gemini não especificado");
    }

    // Modelos válidos baseados na documentação oficial mais recente do Google AI
    const validModels = [
      "gemini-3-flash",
      "gemini-3-pro",
      "gemini-2.5-flash",
    ...
    let modelToUse = aiSettings.model;
    if (!validModels.includes(aiSettings.model)) {
      console.warn(`[GEMINI] Modelo ${aiSettings.model} não disponível. Usando gemini-2.5-flash`);
    ...
    const geminiHistory: Content[] = messagesAI
      .filter(msg => msg.content && msg.content.trim()) // Filtrar mensagens vazias
      .map(msg => ({
        role: msg.role === "assistant" ? "model" : "user",
        parts: [{ text: msg.content }],
      }));

    console.log(`[GEMINI] Histórico: ${geminiHistory.length} mensagens`);
    ...
    if (error.message?.includes("API_KEY_INVALID")) {
      throw new Error("API Key do Gemini inválida. Verifique sua configuração.");
    }
    
    if (error.message?.includes("QUOTA_EXCEEDED")) {
      throw new Error("Limite de uso do Gemini excedido. Tente novamente mais tarde.");
    }
    
    if (error.message?.includes("MODEL_NOT_FOUND") || error.message?.includes("is not found")) {
      throw new Error(`Modelo ${aiSettings.model} não está disponível. Use um dos modelos mais recentes: gemini-2.5-flash, gemini-3-flash, gemini-2.5-pro.`);
    }

    if (error.message?.includes("SAFETY")) {
      throw new Error("Mensagem bloqueada por filtros de segurança. Reformule sua pergunta.");
    }
...
if (minutesElapsed >= aiSettings.completionTimeout) {
    ...
    if (bodyMessage) {
      ...
      try {
        ...
        let errorMessage = "Desculpe, estou com dificuldades técnicas para processar sua solicitação no momento. Por favor, tente novamente mais tarde.";
        
        if (error.message?.includes("API Key")) {
          errorMessage = "🔑 Configuração da IA não encontrada. Entre em contato com o suporte.";
        } else if (error.message?.includes("QUOTA_EXCEEDED")) {
          errorMessage = "⏰ Limite de uso da IA atingido. Tente novamente em alguns minutos.";
        } else if (error.message?.includes("MODEL_NOT_FOUND") || error.message?.includes("is not found")) {
          errorMessage = "🤖 Modelo de IA não disponível. Use 'Gemini 2.5 Flash' ou 'Gemini 3 Flash' nas configurações.";
        } else if (error.message?.includes("SAFETY")) {
          errorMessage = "🛡️ Sua mensagem foi bloqueada por filtros de segurança. Tente reformular sua pergunta.";
        } else if (error.message?.includes("Resposta vazia")) {
          errorMessage = "🤔 Não consegui gerar uma resposta adequada. Pode reformular sua pergunta?";
        }
