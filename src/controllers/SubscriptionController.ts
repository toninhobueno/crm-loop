...
export const createSubscription = async (
  ...
  const valor = Number(parseFloat(price).toLocaleString("pt-br", { minimumFractionDigits: 2 }).replace(",", "."));
  ...
  console.log('Verificando condições para Asaas:', {
    hasToken: !!key_ASAAS_TOKEN,
    valor: valor,
    valorMaiorQue10: valor > 10
  });

  if (key_ASAAS_TOKEN && valor > 10) {
    console.log('Iniciando criação de payment link no Asaas...');
    console.log('Token Asaas configurado:', key_ASAAS_TOKEN ? 'SIM' : 'NÃO');
    console.log('Valor da fatura:', valor);
    ...
    console.log('Dados sendo enviados para Asaas:', {
      url: optionsGetAsaas.url,
      data: optionsGetAsaas.data,
      headers: { ...optionsGetAsaas.headers, access_token: '[HIDDEN]' }
    });


    console.log('Iniciando loop de tentativas para Asaas...');
    let tentativas = 0;
    const maxTentativas = 3;

    while (asaasURL === null && tentativas < maxTentativas) {
      tentativas++;
      try {
        console.log(`Tentativa ${tentativas} de criar payment link no Asaas...`);
        const response = await axios.request(optionsGetAsaas);
        
        console.log('Resposta completa da API Asaas:', {
        ...
        console.log('asaasURL criada com sucesso:', asaasURL);

        // Handle the response here
        // You can proceed with the rest of your code that depends on asaasURL
      } catch (error) {
        console.error(`Erro na tentativa ${tentativas} da API do Asaas:`, {
        ...
        // Se houver erros específicos, vamos logá-los
        if (error.response?.data?.errors) {
          console.error('Erros específicos da API:', error.response.data.errors);
        }

        // Se chegou ao limite de tentativas, quebra o loop
        if (tentativas >= maxTentativas) {
          console.error('Limite de tentativas atingido para API do Asaas');
          break;
        }
    ...
    console.log('Loop de tentativas finalizado. asaasURL:', asaasURL);
      ...
      line_items: [
        {
          price_data: {
            ...
            unit_amount: Math.round(parseFloat(price) * 100), // Valor em centavos
          },
          quantity: 1,
        },
      ],
  ...
  if (key_GERENCIANET_PIX_KEY) {
    ...
    const body = {
      ...
      valor: {
        original: parseFloat(price).toLocaleString("pt-br", { minimumFractionDigits: 2 }).replace(",", ".")
      },
  ...
  console.log('Retornando resposta com URLs:', {
    hasAsaasURL: !!asaasURL,
    asaasURL: asaasURL,
    hasStripeURL: !!stripeURL,
    hasMercadopagoURL: !!mercadopagoURL
  });
