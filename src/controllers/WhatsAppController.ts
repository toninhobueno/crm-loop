...
import { removeUAZApi } from "../libs/uazapi";
import { removeNotificameHub } from "../libs/notificamehub";
...
export const store = async (req: Request, res: Response): Promise<Response> => {
  ...
  logger.info(`🔍 DEBUG WhatsApp Controller - req.body.provider: "${req.body.provider}"`);
  logger.info(`🔍 DEBUG WhatsApp Controller - req.body completo: ${JSON.stringify(req.body, null, 2)}`);
  ...
  // DEBUG: Log para verificar o provider recebido
  logger.info(`🔍 DEBUG WhatsApp Controller - Provider recebido: "${provider}", Channel: "${channel}"`);
...
export const remove = async (
  ...
  // Para NotificameHub, verificar pelo provider independente do canal
  if (whatsapp.provider === "notificamehub") {
    ...
    return res.status(200).json({ message: "NotificameHub connection deleted." });
  }

  if (whatsapp.channel === "whatsapp") {
    // Verificar provider
    if (whatsapp.provider === "uazapi") {
      await removeUAZApi(+whatsappId);
    } else {
