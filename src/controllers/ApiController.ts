...
export const sendToGroup = async (req: Request, res: Response): Promise<Response> => {
  const { whatsappId }: WhatsappData = req.body;
  const { msdelay }: any = req.body;
  const {
    groupId,
  ...
  // Validação do groupId
  const schema = Yup.object().shape({
    groupId: Yup.string()
      .required("Group ID is required")
      .matches(/^[\w\d]+@g\.us$|^[\w\d-]+$/, "Invalid group ID format")
  });

  try {
    await schema.validate({ groupId });
  } catch (err: any) {
    throw new AppError(err.message);
  }
  ...
  // Formatar o groupId para o formato correto
  ...
  try {
    ...
    if (!groupMetadata) {
      throw new AppError("Group not found or bot is not a member");
    }

    if (noRegister) {
      // Envio direto sem registrar no sistema
      if (medias) {
        await Promise.all(
          medias.map(async (media: Express.Multer.File) => {
            const publicFolder = path.resolve(__dirname, "..", "..", "public");
            ...
            const options = await getMessageOptions(media.filename, filePath, companyId.toString(), `\u200e ${bodyMessage}`);
            await wbot.sendMessage(formattedGroupId, options);
        ...
        await wbot.sendMessage(formattedGroupId, {
          text: `\u200e ${bodyMessage}`
        });
      }
    } else {
      // Envio com registro no sistema
      ...
      if (!contact) {
        contact = await Contact.create({
          name: groupMetadata.subject || `Grupo ${formattedGroupId.split("@")[0]}`,
          number: formattedGroupId.split("@")[0],
          isGroup: true,
          companyId,
      ...
      if (medias) {
        ...
        await verifyMediaMessage(sentMessage, ticket, contact, null, false, false, wbot);
      } else {
    ...
    throw new AppError(`Error sending message to group: ${error.message}`);
  }
};
