...
export const index = async (req: Request, res: Response): Promise<Response> => {
  const { searchParam, pageNumber, superOnly } = req.query as IndexQuery;
  const { companyId, profile } = req.user;

  if (superOnly === "true" && !(await requestUserIsSuper(req))) {
...
export const store = async (req: Request, res: Response): Promise<Response> => {
  ...
  if (companyUser) {
    let grantSuperMaster = false;
    if (bodySuper === true || bodySuper === "true") {
      if (!req.user || !(await requestUserIsSuper(req))) {
        throw new AppError("ERR_NO_PERMISSION", 403);
      }
      if ((req.user as { companyId?: number }).companyId !== companyUser) {
        throw new AppError("ERR_NO_PERMISSION", 403);
      }
      grantSuperMaster = true;
    }
...
export const remove = async (
  ...
  if (user.super === true) {
    ...
    if (superCount <= 1) {
      throw new AppError(
        "Não é possível excluir o único usuário master da organização.",
        400
      );
    }
  }
