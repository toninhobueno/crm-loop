...
const SetTicketMessagesAsRead = async (ticket: Ticket): Promise<void> => {
  ...
  if (ticket.whatsappId) {
    ...
    if (["open", "group"].includes(ticket.status) && whatsapp && whatsapp.status === 'CONNECTED' && ticket.unreadMessages > 0) {
      try {
        // Para UAZApi, apenas marcamos as mensagens como lidas no banco
        // A UAZApi não tem endpoint específico para marcar como lido via API
        // (o readchat: true nas mensagens enviadas já faz isso automaticamente)
        if (whatsapp.provider === "uazapi") {
          logger.info(`[SetTicketMessagesAsRead] UAZApi: Marking messages as read for ticket ${ticket.id}`);
          ...
          logger.info(`[SetTicketMessagesAsRead] UAZApi: Messages marked as read for ticket ${ticket.id}`);
          return;
        }
