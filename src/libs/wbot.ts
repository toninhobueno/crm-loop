...
const shouldSuppressBaileysInitTimeout = (args: any[]): boolean => {
  const maybeObj = args.find(a => a && typeof a === "object");
  const maybeMsg = args.find(a => typeof a === "string") as string | undefined;

  const outerMsg = maybeObj?.err?.message || maybeObj?.message || "";
  const innerMsg = maybeObj?.err?.output?.payload?.message || "";
  const text = `${outerMsg} ${innerMsg} ${maybeMsg || ""}`.toLowerCase();

  return text.includes("timed out") && text.includes("init queries");
};

const originalBaileysError = loggerBaileys.error.bind(loggerBaileys);
(loggerBaileys as any).error = (...args: any[]) => {
  if (shouldSuppressBaileysInitTimeout(args)) {
    logger.warn("[WBOT] Baileys init queries timeout (não bloqueante). A sessão segue ativa.");
    return;
  }

  originalBaileysError(...args);
};
